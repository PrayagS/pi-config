...
function applyFilters(prompt: string, rules: FilterRule[]): string {
	return rules.filter((r) => r.enabled).reduce((p, rule) => applyRule(p, rule), prompt);
}
...
export default function systemPromptFilterExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("spf", {
		description: "Manage system prompt filters  (add | remove | toggle | list | show | test)",
	...
	async function handleMenu(ctx: ExtensionContext) {
		...
		const summary = total === 0 ? "no rules yet" : `${active}/${total} rules active`;
		...
		const chosen = await ctx.ui.select(`System Prompt Filters  (${summary})`, options);
		if (!chosen) return;
	...
	async function handlePickFromPrompt(
		...
		for (const raw of selectedRaws) {
			config.rules.push({
				id: generateId(),
				name: raw.trim().replace(/^[-*]\s*/, "").slice(0, 50),
				type: "string",
		...
		while (true) {
			...
			const displayList = [
				...
				...items.map((raw, i) => {
					...
					const mark = selected.has(i) ? "\u2713" : covered ? "\u2605" : "\u25cb";
	...
	async function handleAddManual(ctx: ExtensionContext) {
		...
		const name = await ctx.ui.input("Rule name", "Short label, e.g:  Remove guidelines");
		if (!name?.trim()) return;
	...
	async function handleRemove(ctx: ExtensionContext) {
		if (config.rules.length === 0) {
			ctx.ui.notify("No filter rules to remove. Use /spf add to create one.", "info");
			return;
		}

		const items = config.rules.map(
			(r) => `${r.enabled ? "\u2713" : "\u25cb"} [${r.type}] ${r.name}  \u2014  ${r.pattern.slice(0, 50)}`,
		);
	...
	async function handleToggle(ctx: ExtensionContext) {
		if (config.rules.length === 0) {
			ctx.ui.notify("No filter rules defined. Use /spf add to create one.", "info");
			return;
		}
	...
	async function handleShow(ctx: ExtensionContext) {
		...
		const headings = lines
			...
			.map(({ line, i }) => `  line ${String(i + 1).padStart(4, " ")} \u2502 ${line}`);
		...
		switch (sub) {
			...
			case "write": {
				...
				try {
					...
					ctx.ui.notify(`Failed to write file: ${(err as Error).message}`, "error");
				}
				break;
			}
		}
	...
	async function handleTest(ctx: ExtensionContext) {
		...
		if (enabled.length === 0) {
			ctx.ui.notify("No enabled rules to test. Use /spf add to create one.", "info");
			return;
		}

		const filtered = applyFilters(prompt, enabled);
		const removed = prompt.length - filtered.length;
		const pct = prompt.length > 0 ? Math.round((removed / prompt.length) * 100) : 0;
