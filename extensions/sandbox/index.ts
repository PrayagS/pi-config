...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("sandbox", {
    description:
      "Manage sandbox: /sandbox, /sandbox toggle, /sandbox add <path>, /sandbox remove <path>",
    handler: async (args, ctx) => {
      ...
      if (subcommand === "add") {
        ...
        if (!sandboxEnabled) {
          ctx.ui.notify(
            "Sandbox is disabled. Enable it first with /sandbox toggle",
            "warning"
          )
          return
        }
        ...
        if (choice.includes("read")) {
          if (!sessionAllowRead.includes(pathArg))
            sessionAllowRead.push(pathArg)
        }
        if (choice.includes("write")) {
          if (!sessionAllowWrite.includes(pathArg))
            sessionAllowWrite.push(pathArg)
        }
        ...
        ctx.ui.notify(
          `Granted ${granted} access for "${pathArg}" (session only)`,
          "info"
        )
        return
      }
      ...
      if (subcommand === "remove") {
        ...
        ctx.ui.notify(
          `Removed ${removed.join(" + ")} access for "${pathArg}"`,
          "info"
        )
        return
      }
      ...
      const lines = [
        ...
        ...(sessionAllowRead.length > 0
          ? [`  Session Read: ${sessionAllowRead.join(", ")}`]
          : []),
        ...(sessionAllowWrite.length > 0
          ? [`  Session Write: ${sessionAllowWrite.join(", ")}`]
          : []),
      ]
