...
function detectRepoRoots(cwd: string): string[] {
  ...
  // Try jj (may be colocated with git, so check separately)
  try {
    const jjRoot = execSync("jj root", {
      cwd,
      encoding: "utf-8",
      stdio: ["ignore", "pipe", "ignore"],
    }).trim();
...
export default function (pi: ExtensionAPI) {
    ...
    try {
      ...
      await SandboxManager.initialize({
        ...
        filesystem: {
          ...config.filesystem,
          denyRead: config.filesystem?.denyRead ?? [],
          allowRead: [...(config.filesystem?.allowRead ?? []), ...sessionAllowedReadPaths, ...sessionAllowedWritePaths],
  ...
  async function applyWriteChoice(
    ...
    if (choice === "project") {
      addReadPathToConfig(projectPath, filePath);
      addWritePathToConfig(projectPath, filePath);
    }
    if (choice === "global") {
      addReadPathToConfig(globalPath, filePath);
      addWritePathToConfig(globalPath, filePath);
    }
    await reinitializeSandbox(cwd);
  }
  ...
  pi.registerCommand("sandbox-add", {
    description: "Add a path to allowlist: /sandbox-add <read|write> <path>",
    handler: async (args, ctx) => {
      ...
      if (parts.length < 2) {
        ctx.ui.notify("Usage: /sandbox-add <read|write> <path>", "warning");
        return;
      }
      ...
      if (type !== "read" && type !== "write") {
        ctx.ui.notify("First argument must be 'read' or 'write'", "warning");
        return;
      }

      const choice = await ctx.ui.select(`Add "${pathArg}" to ${type} allowlist:`, [
        "This session only",
        "This project  →  .pi/sandbox.json",
        "All projects  →  ~/.pi/agent/sandbox.json",
        "Cancel",
      ]);
      ...
      if (type === "read") {
        if (choice.startsWith("This session")) {
          if (!sessionAllowedReadPaths.includes(pathArg)) sessionAllowedReadPaths.push(pathArg);
        } else if (choice.startsWith("This project")) {
          addReadPathToConfig(getConfigPaths(ctx.cwd).projectPath, pathArg);
          if (!sessionAllowedReadPaths.includes(pathArg)) sessionAllowedReadPaths.push(pathArg);
        } else {
          addReadPathToConfig(getConfigPaths(ctx.cwd).globalPath, pathArg);
          if (!sessionAllowedReadPaths.includes(pathArg)) sessionAllowedReadPaths.push(pathArg);
        }
      } else {
        // Write access implies read access
        if (choice.startsWith("This session")) {
          if (!sessionAllowedReadPaths.includes(pathArg)) sessionAllowedReadPaths.push(pathArg);
          if (!sessionAllowedWritePaths.includes(pathArg)) sessionAllowedWritePaths.push(pathArg);
        } else if (choice.startsWith("This project")) {
          addReadPathToConfig(getConfigPaths(ctx.cwd).projectPath, pathArg);
          addWritePathToConfig(getConfigPaths(ctx.cwd).projectPath, pathArg);
          if (!sessionAllowedReadPaths.includes(pathArg)) sessionAllowedReadPaths.push(pathArg);
          if (!sessionAllowedWritePaths.includes(pathArg)) sessionAllowedWritePaths.push(pathArg);
        } else {
          addReadPathToConfig(getConfigPaths(ctx.cwd).globalPath, pathArg);
          addWritePathToConfig(getConfigPaths(ctx.cwd).globalPath, pathArg);
          if (!sessionAllowedReadPaths.includes(pathArg)) sessionAllowedReadPaths.push(pathArg);
          if (!sessionAllowedWritePaths.includes(pathArg)) sessionAllowedWritePaths.push(pathArg);
        }
      }

      await reinitializeSandbox(ctx.cwd);
      ctx.ui.notify(`Added "${pathArg}" to ${type} allowlist`, "info");
    },
  });

  pi.registerCommand("sandbox-remove", {
    description: "Remove a path from session allowlist",
    handler: async (_args, ctx) => {
      if (!sandboxEnabled) {
        ctx.ui.notify("Sandbox is disabled", "warning");
        return;
      ...
      const options: string[] = [
        ...sessionAllowedReadPaths.map((p) => {
          const inConfig = matchesPattern(p, configAllowRead);
          return `[read]  ${p}${inConfig ? " (also in config)" : ""}`;
        }),
        ...sessionAllowedWritePaths.map((p) => {
          const inConfig = matchesPattern(p, configAllowWrite);
          return `[write] ${p}${inConfig ? " (also in config)" : ""}`;
        }),
      ];

      if (options.length === 0) {
        ctx.ui.notify("No session-temporary paths to remove", "info");
        return;
      }

      options.push("Cancel");

      const choice = await ctx.ui.select("Remove from session allowlist:", options);
      ...
      if (choice.startsWith("[read]")) {
        const rawPath = choice.slice(8); // "[read]  ".length
        const path = rawPath.replace(" (also in config)", "");
        const idx = sessionAllowedReadPaths.indexOf(path);
        if (idx !== -1) {
          sessionAllowedReadPaths.splice(idx, 1);
          await reinitializeSandbox(ctx.cwd);
          
          // Check if also in config
          if (matchesPattern(path, configAllowRead)) {
            ctx.ui.notify(
              `Removed "${path}" from session, but it's also in config and will remain allowed.\n` +
              `Edit allowRead in:\n  ${projectPath}\n  ${globalPath}`,
              "warning",
            );
          } else {
            ctx.ui.notify(`Removed "${path}" from session read allowlist`, "info");
          }
        }
      } else if (choice.startsWith("[write]")) {
        const rawPath = choice.slice(8); // "[write] ".length
        const path = rawPath.replace(" (also in config)", "");
        const idx = sessionAllowedWritePaths.indexOf(path);
        if (idx !== -1) {
          sessionAllowedWritePaths.splice(idx, 1);
          await reinitializeSandbox(ctx.cwd);
          
          // Check if also in config
          if (matchesPattern(path, configAllowWrite)) {
            ctx.ui.notify(
              `Removed "${path}" from session, but it's also in config and will remain allowed.\n` +
              `Edit allowWrite in:\n  ${projectPath}\n  ${globalPath}`,
              "warning",
            ...
            ctx.ui.notify(`Removed "${path}" from session write allowlist`, "info");
          }
        }
      }
    },
