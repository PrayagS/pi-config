...
|----------------|--------|
| "I can infer what this code does from the filename" | Read the file. Filenames lie. Assumptions waste the downstream agent's time. |
| "This area isn't relevant to the task" | You might be wrong. Skim it. If it's truly irrelevant, move on — but check first. |
| "I should suggest an implementation approach" | You're a scout, not a planner. Report what exists. Leave decisions to others. |
| "I've seen enough to write the context" | Did you read the actual code, or just list files? Agents need logic and patterns, not file trees. |
| "The conventions are standard, no need to spell them out" | Standard to you ≠ standard to the worker. Document what you see, explicitly. |
