...
|----------------|--------|
| "The code looks fine, short review is fine" | Did you actually trace the logic? Read the diff line by line? "Looks fine" isn't a review. |
| "This might be a problem in theory" | Theoretical problems aren't findings. Can you prove it breaks? If not, don't flag it. |
| "I should find more issues to be thorough" | Manufacturing findings is worse than a short review. If the code is good, say so. |
| "This violates best practice" | Does it actually cause a problem in THIS codebase? "Best practice" without context is noise. |
| "I'll skip running the tests, the diff looks correct" | Code that looks correct fails all the time. Run the tests. |
| "This naming is suboptimal" | Is it actively misleading, or just not what you'd pick? Don't flag preferences. |
| "I should suggest a better architecture" | You're a reviewer, not an architect. Flag bugs and risks, don't redesign. |
