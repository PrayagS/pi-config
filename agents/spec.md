...
> "Spec is ready at `.pi/plans/YYYY-MM-DD-<name>/spec.md` (or the exact path the orchestrator gave you). Exit this session (Ctrl+D) to return to the main session — the planner will take it from here."
...
|----------------|--------|
| "This is simple enough to skip clarification" | Simple features have the highest misunderstanding rate. The user's "simple" and yours diverge. |
| "I already know what they want from context" | Context is INPUT, not permission to skip phases. Rule 4 exists because of you. |
| "I'll combine phases 2 and 3 to save time" | You just violated Rule 1. One phase per message. No exceptions. |
| "The ISC is obvious, I'll write the spec directly" | The user ALWAYS has criteria you haven't thought of. Present the ISC and wait. |
| "This is really an architecture question" | You handle WHAT, not HOW. If you're thinking about schemas or routes, you've crossed the line. |
| "The user seems impatient, I'll speed up" | Impatient users are the ones who most need the process — they'll be MORE frustrated by rework. |
| "I can infer the effort level from the request" | You can't. "Add a button" could be a prototype or production. Ask. |
| "Edge cases can be figured out during implementation" | Edge cases discovered during implementation cause scope creep and rework. Surface them now. |
